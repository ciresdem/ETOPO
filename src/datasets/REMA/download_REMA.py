...
REMA_html_url = r'https://data.pgc.umn.edu/elev/dem/setsm/REMA/mosaic/v1.1/{subdir}/'
# These are subdirectories of the above URL

REMA_datasets = ["1km", "8m", "100m", "200m"]


REMA_local_data_dir = os.path.abspath(os.path.join(os.path.split(__file__)[0], "../../../../DEMs/REMA/v1.1/{subdir}"))
URL_list_file = os.path.join(REMA_local_data_dir, "_URL_LIST.txt")

# Find links to sub-directories in the html file, and then explore those.
REMA_subdir_regex = "(?<=<a href=\")(?P<subdir>[\d_]+\/)(?=\"\>(?P=subdir)<\/a>)"
REMA_file_regex = "(?<=<a href=\")(?P<fname>[\w\.]+)(?=\"\>(?P=fname)<\/a>)"

class REMA_Downloader (datasets.download_dataset.DatasetDownloader_BaseClass):
    def __init__(self, dataset_id):
        # Initialized the parent class.
        html_link = REMA_html_url.replace("{subdir}", dataset_id)
        local_dir = REMA_local_data_dir.replace("{subdir}", dataset_id)
    ...
    def create_list_of_links(self,
                             url=None,
                             recurse=True,
                             file_regex=REMA_file_regex,
                             subdir_regex=REMA_subdir_regex,
                             write_to_file=True):
        if url is None:
        ...
        if write_to_file:
            print("Parsing directory structure at", url, "to find REMA files to download. This may take a bit.")
...
def import_and_parse_args():
    ...
    parser.add_argument("-dataset_id", "-d", type=str, default="8m", help="ID of the dataset resolution: Choices are " + ", ".join(REMA_datasets) + ". Default 10m.")
...
if __name__ == "__main__":
    ...
    downloader = REMA_Downloader(dataset_id=args.dataset_id)
