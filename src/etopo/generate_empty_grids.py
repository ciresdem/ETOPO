...
def create_empty_tiles(directory,
    ...
    if dtype_lower not in dtypes_accepted:
        raise TypeError("Unknown value for dtype: '{0}'. Accepted dtype values are:".format(dtype),
                        " ".join(dtypes_accepted))
    else:
        dtype = dtype_lower
    ...
    if int(file_dim_size) != file_dim_size:
        raise UserWarning(
            "The tile size {0}-deg is not evenly divislble by the resolution {1}-sec. Results not guaranteed to work well.".format(
                tile_width_deg, resolution_s))
...
def get_ice_sheet_bed_15deg_bboxes(verbose: bool = True) -> list:
    """For the 15-degree dataset, get bounding boxes that have valid tiles that are in the BedMachine_Bed dataset.

    param verbose: Have outputs to the screen.
    :return: List of bounding boxes. Each box is a 4-length (xmin, ymin, xmax, ymax) tuple.
    :rtype: list
    """
    # Ordinarily I would just import the source_dataset_BedMachine_Bed.py file, but that causes circular inputs. I'll just
    # search for the files more explicitly using the configfile paths and references.
    BM_config = utils.configfile.config(os.path.join(os.path.dirname(__file__),"..","datasets","BedMachine_Bed","BedMachine_Bed_config.ini"))
    data_path = BM_config._abspath(BM_config.source_datafiles_directory_15s)
    files_list = [os.path.join(data_path, fn) for fn in os.listdir(data_path) if re.search(BM_config.datafiles_regex, fn) is not None]
    bbox_list = [None] * len(files_list)
    for i, fname in enumerate(files_list):
        ds = gdal.Open(fname, gdal.GA_ReadOnly)
        xmin, xres, _, ymax, _, yres = ds.GetGeoTransform()
        # These should be integer boundaries, multiples of 15 in this case.
        assert (numpy.round(xmin) == xmin) and (numpy.round(xmin / 15) == xmin / 15)
        assert (numpy.round(ymax) == ymax) and (numpy.round(ymax / 15) == ymax / 15)
        bbox_list[i] = (int(xmin), int(ymax - 15), int(xmin + 15), int(ymax))

    # Sort by ymin, then xmin, for readability
    return sorted(bbox_list, key=lambda x: (x[1],x[0]))

def get_ice_sheet_bed_polygon(verbose: bool = True) -> object:
    """Read the BedMachine_Bed dataset and return a multi-polygon from the valid ice sheet bed tiles.

    NOTE: source_dataset_BedMachine.py should have already been run with extraneous (no-data) files removed.
    Returns a shapely.geometry.Polygon or shapely.Geometry.MultiPolygon"""
    bboxes = get_ice_sheet_bed_15deg_bboxes(verbose=verbose)
