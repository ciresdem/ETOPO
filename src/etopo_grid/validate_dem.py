...
def read_dataframe_file(df_filename):
    ...
    if ext == ".pickle":
        ...
        print("Unknown dataframe file extension '{ext}'. (Currently supporting .pickle, .csv, or .txt)")

    return dataframe

def collect_raw_photon_data(dem_bbox, photon_dataframe_file, granule_ids, overwrite=False, verbose=True):
    ...
    if os.path.exists(photon_dataframe_file) and not overwrite:
        ...
        dataframe = retrieve_land_photons.get_photon_data_multiple_granules(granule_ids,
...
def validate_dem_parallel(dem_name,
    ...
    if results_dataframe_file != None and os.path.exists(results_dataframe_file) and not overwrite:
        ...
        if dem_vertical_datum != output_vertical_datum:
            dem_base, dem_ext = os.path.splitext(dem_name)
            converted_dem_name = dem_base + "_" + output_vertical_datum + dem_ext
        ...
        # If the photon dataframe file containing all the photons in this tile already exists, just use it.
            ...
            if skip_icesat2_download and (len(granules_existing_in_directory) > 0):
                ...
                ATL03_variables = retrieve_land_photons.ATL03_variables_needed
                ...
                ATL08_variables = retrieve_land_photons.ATL08_variables_needed
                # atl08_granules_list =
        ...
        if not quiet:
            print(len(photon_df), "ICESat-2 photons present in photon dataframe.")
        ...
        if not quiet:
            print("{:,}".format(numpy.count_nonzero(dem_goodpixel_mask)), "nonzero land cells exist in the DEM.")
            print("{:,} ICESat-2 photons overlap".format(len(photon_df)),
                  "{:,}".format(N),
                  "DEM cells ({:0.2f}% of total DEM data).".format(numpy.count_nonzero(dem_overlap_mask) * 100 / numpy.count_nonzero(dem_goodpixel_mask)))
    ...
    if plot_results:
        create_result_plots(results_dataframe)
...
def write_summary_stats_file(results_df, statsfile_name, verbose=True):
    lines = []
    lines.append("Number of DEM cells: {0}".format(len(results_df)))
    lines.append("Total number of photons in inter-decile ranges within cells: {0}".format(results_df["numphotons_intd"].sum()))

    mean_diff = results_df["diff_mean"]

    lines.append("Mean bias (m): {0}".format(mean_diff.mean()))
    lines.append("RMSE (m): {0}".format(numpy.sqrt(numpy.mean(numpy.power(mean_diff, 2)))))
    lines.append("== Decile ranges of errors (m) (Look for long-tails, indicating possible artifacts.) ===")
    ...
    for l,v in zip(percentile_levels, percentile_values):
        lines.append("    {0:>3d} %: {1}".format(l, v))

    lines.append("Mean canopy cover (%): {0}".format(results_df["canopy_fraction"].mean()*100))
    lines.append("% of cells with >0 measured canopy (%): {0}".format(len(results_df.canopy_fraction > 0.0) / len(results_df)))
    lines.append("Mean canopy cover in cells with >0 canopy (%): {0}".format(results_df[results_df["canopy_fraction"] > 0].mean()))
    lines.append("Mean roughness (std. dev. of photon elevations within each cell, showing the elevation variations within cell-length distances: {0}".format(results_df["stddev"].mean()))
...
def generate_result_geotiffs(results_dataframe, dem_ds, results_dataframe_filename, verbose=True):
    ...
    """
    result_tif_filename = os.path.splitext(results_dataframe_filename)[0] + "_DEM_ICESat2_differences.tif"
...
def create_result_plots(results_dataframe, verbose=True):
    """Give some summary statistics plots of the results."""
    # TODO: Implement this as well, using plot_validation_results.py
    if verbose:
        print("create_result_plots() not implemented yet. Will fill this in...")
    return
