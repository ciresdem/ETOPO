...
def get_photon_data_multiple_granules(gid_list,
                                      beam=None,
                                      return_type=numpy.ndarray,
                                      bounding_box = None,
                                      parallelize=True,
    ...
    for i,gid in enumerate(gid_list):
        ...
        array_list[i] = get_photon_data(gid, beam=beam, return_type=return_type, bounding_box=bounding_box) # , surface=surface)

        # Print number of photons. Use comma separators in the formatting.
        if verbose:
            print("{:,}".format(array_list[i].shape[0]), "photons.",
...
def get_photon_data(granule_id, granule_dir = None, beam=None, return_type=numpy.ndarray,
                    bounding_box = None):
    ...
    # TODO: Also check to see if this syntax works for the dataframes.
    if bounding_box:
...
def save_photon_data_from_directory(dirname, photon_h5="photons.h5", bounding_box= None, beam=None, verbose=True):
    ...
    dataframe = get_photon_data_multiple_granules(common_granule_ids,
                                                  beam=beam,
...
def output_shapefile_of_granules(shapefile_name, granule_ids_or_dir, bbox=None, verbose=True):
